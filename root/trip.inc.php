...
if (isset($_POST['submit'])){
	...
	if ($origin == "" || $origin == " " || $class == "" || $dest == "" || $dest == " " || $date == "mm/dd/yyyy" || $date == "00/00/0000" || $date == "") {
		mysqli_close($link);
		header("Location: trip.php#planflight?search=empty");
		exit();
	} elseif ($date2 != "00/00/0000" && $date2 != "" && $date2 < $today) {
		mysqli_close($link);
    	header("Location: trip.php#planflight?search=we can't time travel yet, sorry");
		exit();
	
	} elseif ($date2 != "00/00/0000" && $date2 != "" && $date1 > $date2) {
		mysqli_close($link);
    	header("Location: trip.php#planflight?search=return flight cannot be before departure");
		exit();
		...
		if($origin == $dest) {
		mysqli_close($link);
		header("Location: trip.php#planflight?search=Origin cannot equal Destination");	
		exit();
